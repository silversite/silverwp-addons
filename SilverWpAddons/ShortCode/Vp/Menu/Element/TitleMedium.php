 ...
 */
class TitleMedium extends ElementAbstract {
    protected $name = 'tmedium';
    protected $with_close_tag = false;

    public function createElements() {
        $attributes = array(
            $this->text( 'text', Translate::translate( 'Title text' ) ),
            $this->radio( 'align', Translate::translate( 'Align' ), array(
                array(
                    'label' => \SilverWpAddons\Translate::translate( 'center' ),
                    'value' => 'center'
                ),
                array(
                    'label' => \SilverWpAddons\Translate::translate( 'left' ),
                    'value' => 'left'
                ),
            ) ),
        );
        $elements   = $this->addElement( Translate::translate( 'Medium title (H3)' ), $attributes );
