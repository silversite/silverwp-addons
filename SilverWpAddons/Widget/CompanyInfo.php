...
class CompanyInfo extends WidgetAbstract {
	public function __construct() {
		...
		$args['fields'] = array(
			...
			array(
				'name'  => Translate::params( 'Show Mobile phone number %s',
					'#1' ),
	...
	public function widget( $args, $instance ) {
		...
		$out .= isset( $instance['title'] ) && $instance['title']
			? $instance['title']
			: Translate::translate( 'Company Information' );
		$out .= $args['after_title'] . "\n";

		$company['name'] = $instance['show_company_name'] === '1'
			? Option::get_theme_option( 'company_name' ) : '';

		$company['address'] = $instance['show_company_street'] === '1'
			? Option::get_theme_option( 'company_street' ) . '<br />' : '';
		$company['address'] .= $instance['show_company_postcode'] === '1'
			? Option::get_theme_option( 'company_postcode' ) . ' ' : '';
		$company['address'] .= $instance['show_company_city'] === '1'
			? Option::get_theme_option( 'company_city' ) : '';
		$company['address'] .= $instance['show_company_state'] === '1'
			? '<br />' . Option::get_theme_option( 'company_region' ) : '';
		$company['address'] .= $instance['show_company_country'] === '1'
			? '<br />' . Option::get_theme_option( 'company_country' ) : '';
		...
		$company['contact'] = $instance['show_company_phone1'] === '1'
			? '<span class="phone">' . __( 'Phone', 'silverwp' ) . ': '
			  . Option::get_theme_option( 'company_phone1' ) . '</span><br />'
			: '';
		$company['contact'] .= $instance['show_company_phone2'] === '1'
			? '<span class="phone">' . __( 'Phone', 'silverwp' ) . ' #2: '
			  . Option::get_theme_option( 'company_phone2' ) . '</span><br />'
			: '';
		$company['contact'] .= $instance['show_company_email'] === '1'
			? '<span class="email">' . __( 'Email', 'silverwp' )
			  . ': <a href="javascript:location.href=\'mailto:\'+String.fromCharCode('
			  . $email_ASCII . ')+\'?\'">' . $email . '</a></span>' : '';
		$company['contact'] .= $instance['show_company_mobile_phone1'] === '1'
			? '<span class="mobile">' . __( 'Mobile', 'silverwp' ) . ': '
			  . Option::get_theme_option( 'company_mobile1' ) . '</span><br />'
			: '';
		$company['contact'] .= $instance['show_company_mobile_phone2'] === '1'
			? '<span class="mobile">' . __( 'Mobile', 'silverwp' ) . ' #2: '
			  . Option::get_theme_option( 'company_mobile2' ) . '</span><br />'
			: '';
		$company['contact'] .= $instance['show_company_fax'] === '1'
			? '<span class="fax">' . __( 'Fax', 'silverwp' ) . ': '
			  . Option::get_theme_option( 'company_fax' ) . '</span><br />'
			: '';

		$company['hours'] = $instance['show_company_business_hours'] === '1'
			? nl2br( Option::get_theme_option( 'company_business_hour' ) ) : '';

		$out .= $company['contact'] ? '<p class="contact">'
		                              . $company['contact'] . '</p>' : '';
		$out .= $company['name'] ? '<p class="companyname"><strong>'
		                           . $company['name'] . '</strong></p>' : '';
		$out .= $company['address'] ? '<p class="address">'
		                              . $company['address'] . '</p>' : '';
		$out .= $company['hours'] ? '<p class="hours">' . $company['hours']
		                            . '</p>' : '';
